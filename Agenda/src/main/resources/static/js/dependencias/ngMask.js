								...
								return {
									...
									compile : function($element, $attrs) {
										if (!$attrs.mask || !$attrs.ngModel) {
											$log
													.info('Mask and ng-model attributes are required!');
											return;
										}
										...
										function setSelectionRange(
											...
											// using $timeout:
											// it should run after the DOM has
											// been manipulated by Angular
											// and after the browser renders
											// (which may cause flicker in some
											// cases)
											...
											pre : function($scope, $element,
												...
												promise = maskService
														.generateRegex({
															...
															// length
															limit : (($attrs.limit
																	|| $attrs.maskLimit || 'true') === 'true'),
															// how to act with a
															// wrong value
															restrict : ($attrs.restrict
																	|| $attrs.maskRestrict || 'select'), // select,
																											// reject,
																											// accept
															// set validity mask
															validate : (($attrs.validate
																	|| $attrs.maskValidate || 'true') === 'true'),
															// default model
															// value
															model : $attrs.ngModel,
															// default input
												...
												promise
														.then(function() {
															...
															function parseViewValue(
																...
																// values capped
																// used on view
																var viewValueWithDivisors = viewValue
																		.withDivisors(true);
																// used on model
																var viewValueWithoutDivisors = viewValue
																		.withoutDivisors(true);

																try {
																	// get
																	// current
																	// regex
																	var regex = maskService
																			.getRegex(viewValueWithDivisors.length - 1);
																	var fullRegex = maskService
																			.getRegex(maskWithoutOptionals.length - 1);

																	// current
																	// position
																	// is valid
																	var validCurrentPosition = regex
																			.test(viewValueWithDivisors)
																			|| fullRegex
																					.test(viewValueWithDivisors);
																	...
																	if (options.restrict !== 'accept') {
																		if (options.restrict === 'select'
																				&& (!validCurrentPosition || diffValueAndViewValueLengthIsOne)) {
																			var lastCharInputed = value[(value.length - 1)];
																			var lastCharGenerated = viewValueWithDivisors[(viewValueWithDivisors.length - 1)];

																			if ((lastCharInputed !== lastCharGenerated)
																					&& diffMaskAndViewValueIsGreaterThanZero) {
																				viewValueWithDivisors = viewValueWithDivisors
																						+ lastCharInputed;
																			}

																			var wrongPosition = maskService
																					.getFirstWrongPosition(viewValueWithDivisors);
																			if (angular
																					.isDefined(wrongPosition)) {
																				setSelectionRange(wrongPosition);
																			}
																		} else if (options.restrict === 'reject'
																				&& !validCurrentPosition) {
																			viewValue = maskService
																					.removeWrongPositions(viewValueWithDivisors);
																			viewValueWithDivisors = viewValue
																					.withDivisors(true);
																			viewValueWithoutDivisors = viewValue
																					.withoutDivisors(true);
					...
					[
							...
							function($q, OptionalService, UtilService) {
								function create() {
									...
									function generateIntermetiateRegex(i,
											forceOptional) {
										...
										try {
											...
											if (hasOptional
													&& ((i + 1) < maskWithoutOptionalsLength)) {
											...
											elementOptionalRegex = new RegExp(
													currentRegex);
										} catch (e) {
											throw e;
										}
										return {
											...
											elementOptionalRegex : function() {
												// from element regex, gets the
												// flow of regex until first not
												// optional
									...
									function generateRegex(opts) {
										...
										try {
											...
											optionalIndexes = OptionalService
													.getOptionals(mask)
													.fromMaskWithoutOptionals();
											options['maskWithoutOptionals'] = maskWithoutOptionals = OptionalService
											...
											for (var i = 0; i < maskWithoutOptionalsLength; i++) {
												...
												var elementRegex = charRegex
														.elementRegex();
												var elementOptionalRegex = charRegex
														.elementOptionalRegex();

												var newRegex = cumulativeRegex ? cumulativeRegex.source
														+ elementOptionalRegex.source
														: elementOptionalRegex.source;
												newRegex = new RegExp(newRegex);
												cumulativeRegex = cumulativeRegex ? cumulativeRegex.source
														+ elementRegex.source
														: elementRegex.source;
												cumulativeRegex = new RegExp(
														cumulativeRegex);
									...
									function isOptional(currentPos) {
										return UtilService.inArray(currentPos,
												optionalIndexes);
									}
									...
									function generateOptionalDivisors() {
										...
										var sortedDivisors = divisors
												.sort(sortNumber);
										var sortedOptionals = optionalIndexes
												.sort(sortNumber);
										for (var i = 0; i < sortedDivisors.length; i++) {
											...
											for (var j = 1; j <= sortedOptionals.length; j++) {
												...
												if (optionalDivisors[divisor]) {
													optionalDivisors[divisor] = optionalDivisors[divisor]
															.concat(divisor - j);
												} else {
													optionalDivisors[divisor] = [ (divisor - j) ];
												}

												// get the original divisor for
												// alternative divisor
									...
									function insertDivisors(array, combination) {
										...
										var divs = divisors
												.filter(function(it) {
													var optionalDivisorsKeys = Object
															.keys(
																	optionalDivisors)
															.map(
																	function(it) {
									...
									function tryDivisorConfiguration(value) {
										...
										if (optionalIndexes.length > 0) {
											var lazyArguments = [];
											var optionalDivisorsKeys = Object
													.keys(optionalDivisors);

											// get all optional divisors as
											// array of arrays [[], [], []...]
														...
														.lazyProduct(
																...
																function() {
																	...
																	optionalDivisorsCombinations
																			.push(Array.prototype.slice
																					.call(arguments));
																});
											}

											for (var i = optionalDivisorsCombinations.length - 1; i >= 0; i--) {
												var outputClone = angular
														.copy(output);
												outputClone = insertDivisors(
														outputClone,
														optionalDivisorsCombinations[i]);

												// try validation
												var viewValueWithDivisors = outputClone
														.join('');
											...
											return {
												withDivisors : function(capped) {
													if (capped) {
														return output
																.substr(0,
																		maskWithoutOptionalsLength);
													} else {
														return output;
													}
												},
												withoutDivisors : function(
														capped) {
													if (capped) {
									...
									function getWrongPositions(
											viewValueWithDivisors, onlyFirst) {
										var pos = [];
										...
										for (var i = 0; i < viewValueWithDivisors.length; i++) {
											...
											var value = viewValueWithDivisors
													.substr(0, (i + 1));
									...
									function getFirstWrongPosition(
											viewValueWithDivisors) {
										return getWrongPositions(
												viewValueWithDivisors, true)[0];
									}

									function removeWrongPositions(
											viewValueWithDivisors) {
										var wrongPositions = getWrongPositions(
												viewValueWithDivisors, false);
										var newViewValue = viewValueWithDivisors;

										for (var i = 0; i < wrongPositions.length; i++) {
											var wrongPosition = wrongPositions[i];
											var viewValueArray = viewValueWithDivisors
													.split('');
											viewValueArray.splice(
													wrongPosition, 1);
											newViewValue = viewValueArray
													.join('');
										}
